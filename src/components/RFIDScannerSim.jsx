...
export const RFIDScannerSim = ({
    ...
    if (!matchedUser) {
      setErrorMessage(
        onlyQrMode
          ...
          : `RFID access token code [${trimmedRfid}] is unregistered inside the database.`,
      );
  ...
  const handleUpdateServices = () => {
    ...
    const printPageDelta =
      selectsPrinting && previouslyPrinting
        ? Math.max(0, pagesToPrint - activePrintingPages)
        : 0;

    currentActiveSessions.forEach((session) => {
      const sessionServices = session.services || [];
      const stillSelected = sessionServices.some((serviceName) =>
        selectedServices.includes(serviceName),
      );
      onUpdateLog({
        ...session,
        terminalLocation,
        pagesPrinted: session.pagesPrinted,
        ...(stillSelected
          ? {}
          : {
              checkOutTime: now,
    ...
    addedServices.forEach((serviceName, index) => {
      onAddLog({
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          {[
                            ...
                            },
                            {
                              ...
                              defaultServiceId: "biwag",
                              defaultServiceName: "Discussion Room 1 (BIWAG)",
                            },
                            {
                              id: "3f",
                              label: "3rd Floor",
                            ...
                            },
                            {
                              ...
                              defaultServiceId: "printing",
                              defaultServiceName: "Printing & Xerox",
                            },
                          ].map((floor) => {
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  setTerminalLocation(floor.location);
                                  setSelectedServices([floor.defaultServiceName]);
                                }}
